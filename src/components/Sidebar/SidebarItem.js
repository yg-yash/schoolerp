...
const SidebarItem = ({
  ...
  useMountEffect(() => {
    if (!currentPath || TwoactiveRoute === index || route.path === '/') return;
    toggleTwoMenu(index);
  });
  let isRole;
  if (route.roles) {
    isRole = route.roles.find((role) => localStorage.getItem('role') === role);
  }
  const badge = (badge) => {
    if (!badge) return;
      ...
      >
        ...
        >
          {route.children.map((subitem, index) => {
            let isAuth;
            if (subitem.roles) {
              isAuth = subitem.roles.find((role) => role === isRole);
            }
            if (!isAuth) {
              return null;
            }
            return (
              <>
                ...
                <Collapse
                  in={TwoactiveRoute === index ? true : false}
                  timeout="auto"
                  unmountOnExit
                >
                  <List disablePadding>
                    {subitem.children.map((innerItem, innerIndex) => {
                      let isInnerAuth;
                      if (innerItem.roles) {
                        isInnerAuth = innerItem.roles.find(
                          (role) => role === isRole
                        );
                      }
                      if (!isInnerAuth) {
                      ...
                      return (
                        ...
                        >
                          <ListItem className={classes.menuSubItem} button>
                            <Typography variant="body1" className="flexSpacer">
                              {capitalize(innerItem.name)}
                            </Typography>
                            {badge(innerItem.badge)}
                          </ListItem>
                        </NavLink>
                      );
                    })}
