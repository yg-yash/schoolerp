...
const Sidebar = ({ opened, toggleDrawer, routes, location }) => {
  ...
  const [TwoactiveRoute, setTwoActiveRoute] = useState(undefined);

  const toggleMenu = (index) =>
    setActiveRoute(activeRoute === index ? undefined : index);
  const toggleTwoMenu = (index) =>
    setTwoActiveRoute(TwoactiveRoute === index ? undefined : index);
      ...
      {routes.map((route, index) => {
        ...
        return (
          <SidebarItem
            ...
            toggleTwoMenu={toggleTwoMenu}
            currentPath={isCurrentPath}
            TwoactiveRoute={TwoactiveRoute}
          />
        );
      })}
    </List>
