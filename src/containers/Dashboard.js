  ...
  const getRoutes = (
    <Switch>
      {routes.items.map((item, index) => {
        let isRole;
        if (item.roles) {
          isRole = item.roles.find(
            (role) => localStorage.getItem('role') === role
          );
        }

        return item.type === 'external' && isRole ? (
          <Route
            exact
            path={item.path}
          ...
          item.children.map((subItem) => {
            ...
            if (isAuth) {
              return subItem.children.map((innerItem, index) => {
                return (
                  <Route
                    exact
                    path={`${item.path}${subItem.path}${innerItem.path}`}
                    component={innerItem.component}
                    name={innerItem.name}
                    key={index}
                  />
                );
